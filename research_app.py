    ...
    if st.session_state.get('competency_evaluated', False):
        ...
        with col1:
            ...
            <div class="persona-card similar">
                ...
                <p><strong>{t('competency.tasks_similar')}</strong></p>
            </div>
            """, unsafe_allow_html=True)
            ...
            if rec_info.get('math_engine_used') and rec_info.get('similar_score_info'):
                with st.expander("🔬 " + t("competency.score_details")):
                    ...
                    clt = score_info.get('clt_analysis', {})
                    base_score = score_info.get('base_score', score_info['total_score'])
                    clt_modifier = score_info.get('clt_modifier', 1.0)
                    st.markdown(f"""
                    **7 Aşamalı Hesaplama Sonuçları (CLT Entegre):**

                    **Toplam Skor:** {score_info['total_score']:.3f} / 1.000
                    - Temel Skor: {base_score:.3f}
                    - CLT Çarpanı: ×{clt_modifier:.2f}
                    ...
                    **Bilişsel Yük Analizi (Sweller, 1988):**
                    - 🧠 Intrinsic Load: {clt.get('intrinsic_load', 0):.3f}
                    - ⚠️ Extraneous Load: {clt.get('extraneous_load', 0):.3f}
                    - ✅ Germane Load: {clt.get('germane_load', 0):.3f}
                    - 📦 Toplam Yük: {clt.get('total_load', 0):.3f}
                    - 🎯 Optimal Zone: {'✅ Evet' if clt.get('is_in_optimal_zone') else '❌ Hayır'}

                    **Formül:** R = (α·S + β·C + γ·P + δ·L) × CLT_modifier
                    ...
                    **Güven Aralığı (95% CI):** [{score_info['confidence_interval']['lower']:.3f}, {score_info['confidence_interval']['upper']:.3f}]
                    """)
                    if clt.get('adjustments'):
                        st.markdown("**CLT Düzeltmeleri:** " + ", ".join(clt['adjustments']))
        ...
        with col2:
            ...
            <div class="persona-card complementary">
                ...
                <p><strong>{t('competency.tasks_comp')}</strong></p>
            </div>
            """, unsafe_allow_html=True)
            ...
            if rec_info.get('math_engine_used') and rec_info.get('complementary_score_info'):
                with st.expander("🔬 " + t("competency.score_details")):
                    ...
                    **Formül:** R = (α·(1-S) + β·D + γ·P + δ·L) × CLT_modifier
