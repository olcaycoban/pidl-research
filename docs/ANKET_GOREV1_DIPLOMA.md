# Görev 1 — Diploma Doğrulama Sistemi
**Zorluk:** Düşük | **Blok:** 1 (Adaptif) / 2 (Sabit) | **Mod:** NASA-TLX'e göre dinamik (Blok 1) — UUID paritesine göre sabit (Blok 2)

---

## ÖN-TEST (Görev başlamadan önce) — 5 Soru

**Katılımcı No:** __________ | **Tarih:** __________

---

**S1.** Blockchain'de diploma saklamanın avantajı nedir?

- [ ] A) Değiştirilemez kayıt
- [ ] B) Daha ucuz
- [ ] C) Daha hızlı
- [ ] D) Bilmiyorum

---

**S2.** Hash fonksiyonu ne işe yarar?

- [ ] A) Şifreleme yapar
- [ ] B) Benzersiz parmak izi oluşturur
- [ ] C) Veri sıkıştırır
- [ ] D) Bilmiyorum

---

**S3.** `onlyOwner` modifier'ı eğitim kurumları için neden önemlidir?

- [ ] A) Sadece yetkili kişi diploma ekleyebilsin diye
- [ ] B) Gas maliyetini azaltır
- [ ] C) Doğrulama hızlandırır
- [ ] D) Bilmiyorum

---

**S4.** Akıllı sözleşmede `address` tipi ne ifade eder?

- [ ] A) Ethereum cüzdan veya sözleşme adresi
- [ ] B) IP adresi
- [ ] C) E-posta adresi
- [ ] D) Bilmiyorum

---

**S5.** `mapping(address => bool)` veri yapısı ne sağlar?

- [ ] A) Her adres için O(1) hızında erişim
- [ ] B) Tüm adresleri listeler
- [ ] C) Adresleri doğrular
- [ ] D) Bilmiyorum

---

**Ön-Test Skoru:** _____ / 5

---

## SON-TEST (Görev tamamlandıktan sonra) — 5 Soru

**Katılımcı No:** __________ | **Tarih:** __________

---

**S1.** Blockchain'de diploma saklamanın avantajı nedir?

- [ ] A) Değiştirilemez kayıt
- [ ] B) Daha ucuz
- [ ] C) Daha hızlı
- [ ] D) Bilmiyorum

---

**S2.** Hash fonksiyonu ne işe yarar?

- [ ] A) Şifreleme yapar
- [ ] B) Benzersiz parmak izi oluşturur
- [ ] C) Veri sıkıştırır
- [ ] D) Bilmiyorum

---

**S3.** `onlyOwner` modifier'ı eğitim kurumları için neden önemlidir?

- [ ] A) Sadece yetkili kişi diploma ekleyebilsin diye
- [ ] B) Gas maliyetini azaltır
- [ ] C) Doğrulama hızlandırır
- [ ] D) Bilmiyorum

---

**S4.** Diploma hash'i yerine tüm diploma verisini saklamak ne gibi sorunlar yaratır?

- [ ] A) Gas maliyeti artar
- [ ] B) Gizlilik sorunu oluşur
- [ ] C) Her ikisi de
- [ ] D) Bilmiyorum

---

**S5.** Sahte diploma ile mücadelede blockchain nasıl yardımcı olur?

- [ ] A) Değiştirilemez kayıt sağlar
- [ ] B) Belgeleri daha ucuza basar
- [ ] C) İşlemleri hızlandırır
- [ ] D) Bilmiyorum

---

**Son-Test Skoru:** _____ / 5

**Öğrenme Kazanımı (Son − Ön):** _____ puan

---

## Cevap Anahtarı

| | Ön-Test | Son-Test |
|-|---------|---------|
| S1 | A | A |
| S2 | B | B |
| S3 | A | A |
| S4 | A | C |
| S5 | A | A |
